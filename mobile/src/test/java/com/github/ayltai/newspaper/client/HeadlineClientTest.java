...
public final class HeadlineClientTest extends NetworkTest {
    ...
    public void setUp() throws Exception {
        ...
        Mockito.doReturn(Observable.error(new RuntimeException("Fake error 1"))).when(this.apiService).getFeed(HeadlineClientTest.ERROR_URL);
        Mockito.doReturn(Observable.error(new RuntimeException("Fake error 2"))).when(this.apiService).getHtml(HeadlineClientTest.ERROR_DETAILS_URL);
    ...
    public void Given_headlineUrl_When_getItemsIsCalled_Then_itemsAreReturned() {
        ...
        Assert.assertEquals("Incorrect item description", "教育局副局長蔡若蓮長子潘匡仁，昨晨在家中伸腳出窗危坐，女傭發現並將他拉回屋內，他其後返回房間並將門上鎖，未幾墮樓，倒臥平台，送院後證實死亡。潘生前任職物理治療師，平日喜歡運動和玩樂器，去年在單車賽中撞傷頭部。警方表示，死者生前有情緒病紀錄，據悉是抑鬱症。蔡若蓮驚聞噩耗，到醫院見兒子最後一面，離開時神情哀傷，她感謝各界人士關心，希望各界給予空間讓她和家人渡過困難時間。\n" +
            ...
            "　　蔡若蓮事後得悉長子墮樓重傷趕往醫院，惜抵醫院時其子已證實死亡，母子天人永隔。她見兒子最後一面，離開醫院時顯得哀傷，由男女陪同登車離去。相關新聞刊P4版", items.get(0).getDescription());
    }

    @Test
    public void Given_errorUrl_When_getItemsIsCalled_Then_noItemsAreReturned() {
        final List<NewsItem> items = this.client.getItems(HeadlineClientTest.ERROR_URL).blockingGet();
    ...
    public void Given_headlineDetailsErrorUrl_When_updateItemIsCalled_noItemIsUpdated() {
        final NewsItem newsItem = new NewsItem();
        newsItem.setLink(HeadlineClientTest.ERROR_DETAILS_URL);
